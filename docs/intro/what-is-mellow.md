...
The **Frozen Mellow 2.9 Core Profile** is the current stable language contract. MellowLang 2.9.5 is the current implementation release of that profile.
...
MellowLang 2.9.5 uses the native C VM as the default engine for `mellow run`. It falls back to the Python VM when a script requests features that do not yet have native parity, such as debugger, events, or record/replay.
